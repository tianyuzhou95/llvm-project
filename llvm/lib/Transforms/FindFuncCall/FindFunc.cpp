...
namespace {
    struct FindFunc : public FunctionPass {
        ...
        FindFunc() : FunctionPass(ID) {
            ...
            // ****
            // this test will find that there exists some NULL function call inside AbortableMutex.Lock()
            // call {}* %.field.ld.0(i ....
            // ****
            ...
            for (BasicBlock &bb : F) {
                ...
                for (Instruction &i : bb) {
                    ...
                    if(depth < MAX_DEPTH) {
                        ...
                        temp_sum = getFuncCallSites(*FT, depth+1);
